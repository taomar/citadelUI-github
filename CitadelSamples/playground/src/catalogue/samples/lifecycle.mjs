          ...
          step.cli({
            ...
            command: {
              ...
              args: [
                ...
                'delete',
                '--uri',
                `/subscriptions/${ctx.get('hub.subscriptionId')}/resourceGroups/${rg}/providers/Microsoft.ApiManagement/service/${apim}/subscriptions/${contract.subscriptionName}?api-version=2022-08-01`,
                '--headers',
                'If-Match=*',
              ],
        ...
        for (const [index, asset] of assets.entries()) {
          steps.push(
            step.cli({
              ...
              command: {
                executable: 'az',
                args: ['apim', 'api', 'delete', '-g', rg, '-n', apim, '--api-id', asset.name, '--yes'],
              },
              produces: ['apiDeleted'],
            }),
          );
              ...
              step.cli({
                ...
                command: {
                  ...
                  args: [
                    ...
                    'delete',
                    '--uri',
                    `/subscriptions/${ctx.get('hub.subscriptionId')}/resourceGroups/${rg}/providers/Microsoft.ApiManagement/service/${apim}/backends/${asset.name}-backend?api-version=2022-08-01`,
                    '--headers',
                    'If-Match=*',
                  ],
      ...
      if (ctx.self('deleteWeatherSourceApi')) {
        steps.push(
          step.cli({
            ...
            command: {
              executable: 'az',
              args: ['apim', 'api', 'delete', '-g', rg, '-n', apim, '--api-id', 'weather-api', '--yes'],
            },
