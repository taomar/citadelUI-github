...
test('real MSAL orchestration exchanges PKCE once, validates signed OIDC and selects the exact cached user', async () => {
  ...
  session = sessions.finish(session, await auth.finish(tx, callback.searchParams.get('code')), tx);
  ...
  sessions.consume(badPkce.state, badPkce.correlation);
  await assert.rejects(auth.finish(badPkce, badCallback.searchParams.get('code')));
  sessions.revoke(sessions.get(badPkce.sessionId));
  ...
  session = sessions.finish(session, await auth.finish(azure, azureCallback.searchParams.get('code')), azure);
