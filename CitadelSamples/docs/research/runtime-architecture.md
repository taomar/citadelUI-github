...
| Protected source UI | View-model tests retain exact supplied cell text, force `editable: false`, and reject stale notebook digests or editable responses. `GET /api/source/<id>` verifies the imported notebook digest, joins cited cell source without normalization, and returns cell bytes and hashes for all 19 scenarios. | Exact immutable source delivery is implemented and tested end to end locally. |
| Compile-only validation | `POST /api/source/<id>/validate` accepts only the protocol version, selects source server-side, invokes a shipped stdlib-only validator with `-I -S`, compiles without import or execution, and removes its temporary workspace in `finally`. A real local Python sweep compiled all 19 recipe selections. | The parser/compile-only operation and its offline/no-network/no-live-evidence result boundary are locally implemented and proved. The route remains unavailable in process-free preview and relay modes. |
